...
- [x] 4.1 编写 `tests/unit/test_constraints_executor.py::test_insert_rejects_null_on_not_null`，红
- [x] 4.2 在 `execute_insert` 加 NOT NULL 校验（在类型校验后落盘前）
- [x] 4.3 编写 `test_insert_rejects_duplicate_unique_key`，红
- [x] 4.4 实现 UNIQUE 单列 + 复合键校验（全表扫描）
- [x] 4.5 编写 `test_insert_rejects_duplicate_primary_key`，红（PRIMARY KEY 走同一路径）
- [x] 4.6 实现 PRIMARY KEY 等价 NOT NULL + UNIQUE 合并检查（executor 内对 PK 列同时跑 null 和 unique 校验）
...
- [x] 5.3 在 REPL/CLI 路径上把 `ConstraintViolation` 渲染为单行 `ERROR: ConstraintViolation(kind=..., column=...)`

## 6. 兼容性

- [x] 6.1 编写 fixture：MVP 旧版 `.db`（无约束 schema）反序列化路径不能爆
- [x] 6.2 编写 `test_catalog_old_file_migration_loads_with_nullable_default_true`，绿
- [x] 6.3 验证：MVP 234 个测试 + engine-v1 后续测试全部继续通过

## 7. 性能与回归

- [x] 7.1 计时 fixture：n=1000 行 INSERT 全部通过 O(n) UNIQUE 校验总耗时 < 100ms
